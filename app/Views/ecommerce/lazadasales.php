...
<div class="container">
    ...
    <?php
        ...
        if(!empty($_SESSION['saleslaz_updated'])){
            echo '<div class="alert alert-success" role="alert">
            <h4 class="alert-heading">Update Success!</h4>
            <p>Lazada sales has been successfully updated</p>
        </div>';
            unset($_SESSION['saleslaz_updated']);
        }

        if(!empty($_SESSION['saleslaz_pending'])){
            echo '<div class="alert alert-success" role="alert">
            <h4 class="alert-heading">Update Success!</h4>
            <p>Lazada sales has been successfully set to Pending</p>
        </div>';
            unset($_SESSION['saleslaz_pending']);
        }

        if(!empty($_SESSION['saleslaz_delivered'])){
            echo '<div class="alert alert-success" role="alert">
            <h4 class="alert-heading">Update Success!</h4>
            <p>Lazada sale has been successfully set to Delivered</p>
        </div>';
            unset($_SESSION['saleslaz_delivered']);
        }

        if(!empty($_SESSION['saleslaz_cancelled'])){
            echo '<div class="alert alert-success" role="alert">
            <h4 class="alert-heading">Update Success!</h4>
            <p>Lazada sale has been successfully set to Cancelled</p>
        </div>';
            unset($_SESSION['saleslaz_cancelled']);
        }

        if(!empty($_SESSION['saleslaz_missing'])){
            echo '<div class="alert alert-success" role="alert">
            <h4 class="alert-heading">Update Success!</h4>
            <p>Lazada sale has been successfully set to Missing</p>
        </div>';
            unset($_SESSION['saleslaz_missing']);
        }

        if(!empty($_SESSION['salesshop_completeorder'])){
            echo '<div class="alert alert-success" role="alert">
            <h4 class="alert-heading">Update Success!</h4>
            <p>Shopee sale has been successfully set to Completed</p>
        </div>';
            unset($_SESSION['salesshop_completeorder']);
        }
    ?>
    
    <h1><?= $stats; ?> Lazada Sales <i class="fak fa-lazada"></i></h1>
                ...
                <td>
                    <div class="btn-group">
                        ...
                        <?php if(in_array('edit-sales', $arr)){?>
                            <a href="<?= site_url().'lazada/edit/'.str_ireplace(['/','+'],['~','$'],$encrypter->encrypt($as['sales_id'])); ?>/<?= str_ireplace(['/','+'],['~','$'],$encrypter->encrypt($as['customer_id']));?>" class="btn btn-primary btn-icon btn-sm"><i class="fas fa-edit"></i></a>
                        <?php }?>
                    </div>
                    ...
                    <div class="modal fade" id="modalview<?= $as['sales_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                            ...
                            <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel">Lazada Sales</h5>
                            ...
                            <div class="modal-footer">
                                <?= form_open("lazada/markpending/".str_ireplace(['/','+'],['~','$'],$encrypter->encrypt($as['sales_id'])))?>
                                    <button type="submit" class="btn btn-primary">Mark Pending</button>
                                <?= form_close()?>
                                <?= form_open("lazada/markdelivere/".str_ireplace(['/','+'],['~','$'],$encrypter->encrypt($as['sales_id'])))?>
                                    <button type="submit" class="btn btn-success">Mark Delivered</button>
                                <?= form_close()?>
                                <?= form_open("lazada/markcancelled/".str_ireplace(['/','+'],['~','$'],$encrypter->encrypt($as['sales_id'])))?>
                                    <button type="submit" class="btn btn-warning">Mark Cancelled</button>
                                <?= form_close()?>
                                <?= form_open("lazada/markmissing/".str_ireplace(['/','+'],['~','$'],$encrypter->encrypt($as['sales_id'])))?>
                    ...
                    <div class="modal fade" id="modalcomplete<?= $as['sales_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                            ...
                            <div class="modal-body">
                                    <?= form_open("lazada/completeorderlaz")?>
                                    <table class="table table-bordered table-sm">
                                        <thead>
                                            <tr>
                                                ...
                                                <th class="align-middle text-center" style="width: 8%;" rowspan="2">Payment Fee</th>
                                                <th class="align-middle text-center" style="width: 8%;" rowspan="2">Commission Fee</th>
                                        ...
                                        <tbody>
                                            <?php 
                                                ...
                                                $oc = 0;
                                                $payout = 0;
                                                foreach($ecommerce_model->getItemsLaz($as['sales_id']) as $eitem){
                                                $subpayout = 0;
                                                $tq += $eitem['quantity'];
                                                $sfc += $eitem['shipping_fee_collected'];
                                                $sfch += $eitem['shipping_fee_charged'];
                                                $pf += $eitem['payment_fee'];
                                                $cf += $eitem['commission_fee'];
                                                $of += $eitem['other_fee'];
                                                $oc += $eitem['other_credit'];

                                                $subpayout += (($eitem['quantity'] * $eitem['price']) + $eitem['shipping_fee_collected'] + $eitem['other_credit']) - ($eitem['shipping_fee_charged'] + $eitem['payment_fee'] + $eitem['commission_fee'] + $eitem['other_fee']);
                                                $payout += $subpayout;
                                            ?>
                                            <tr>
                                                <td class="align-middle text-center"><input type="hidden" name="sales-lazada[]" value="<?= $eitem['sales_item_id'];?>"></td>
                                                <td>
                                                ...
                                                <td class="align-middle"><input class="form-control form-control-sm text-center" type="number" name="payment-fee[]" value="<?= $eitem['payment_fee'];?>" step="0.01"></td>
                                                <td class="align-middle"><input class="form-control form-control-sm text-center" type="number" name="commission-fee[]" value="<?= $eitem['commission_fee'];?>" step="0.01"></td>
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td data-name="payment-fee" class="text-center"><?= $pf;?></td>
                                                <td data-name="commission-fee" class="text-center"><?= $cf;?></td>
